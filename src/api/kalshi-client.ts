...
function toMarket(km: KalshiMarket): Market {
  ...
  let yesPrice: number;
  const yesBidD = parseFloat(km.yes_bid_dollars as string);
  const yesAskD = parseFloat(km.yes_ask_dollars as string);
  const lastD   = parseFloat(km.last_price_dollars as string);
